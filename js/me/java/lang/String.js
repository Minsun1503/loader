js2me.createClass({
	 ...
	 */
	$split$Ljava_lang_String_$_Ljava_lang_String_: function (regex) {
		if (regex == null) {
			throw new javaRoot.$java.$lang.$NullPointerException();
		}
		var parts = this.text.split(regex.text);
		var result = [];
		for (var i = 0; i < parts.length; i++) {
			result.push(new javaRoot.$java.$lang.$String(parts[i]));
		}
		return result;
	},
	/*
	 ...
	 */
	$format$Ljava_lang_String__Ljava_lang_Object_$Ljava_lang_String_: function (format, args) {
		var str = format.text;
		var i = 0;
		str = str.replace(/%[0-9]*[a-zA-Z]/g, function (match) {
			if (i < args.length) {
				var arg = args[i++];
				if (arg == null) return "null";
				if (arg.text !== undefined) return arg.text;
				if (arg.$toString$$Ljava_lang_String_) return arg.$toString$$Ljava_lang_String_().text;
				if (arg.value !== undefined) return arg.value;
				return arg.toString();
			}
			return match;
		});
		return new javaRoot.$java.$lang.$String(str);
	},
	/*
	 * public static String join(CharSequence delimiter, Iterable elements)
	 */
	$join$Ljava_lang_CharSequence_Ljava_lang_Iterable_$Ljava_lang_String_: function (delimiter, elements) {
		var delim = delimiter.text != null ? delimiter.text : delimiter.$toString$$Ljava_lang_String_().text;
		var iterator = elements.$iterator$$Ljava_util_Iterator_();
		var arr = [];
		while (iterator.$hasNext$$Z()) {
			var elem = iterator.$next$$Ljava_lang_Object_();
			if (elem == null) {
				arr.push("null");
			} else if (elem.text !== undefined) {
				arr.push(elem.text);
			} else if (elem.$toString$$Ljava_lang_String_) {
				arr.push(elem.$toString$$Ljava_lang_String_().text);
			} else {
				arr.push(elem.toString());
			}
		}
		return new javaRoot.$java.$lang.$String(arr.join(delim));
	},
	/*
