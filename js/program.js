...
js2me.generateProgram = function (data) {
		...
		try {
			...
			return function (context) {
				...
				if (isVirtual) {
					...
					if (obj[methodName] == null && methodName[0] !== '$' && obj['$' + methodName] != null) {
						methodName = '$' + methodName;
					}
					if (obj[methodName] == null) {
						var errMsg = "Method undefined: " + (obj.className || "Unknown") + "." + methodInfo.name;
						console.error(errMsg);
						js2me.showError(errMsg);
					...
					var methodName = methodInfo.name;
					if (classObj.prototype[methodName] == null && methodName[0] !== '$' && classObj.prototype['$' + methodName] != null) {
						methodName = '$' + methodName;
					}
					if (classObj.prototype[methodName] == null) {
						var errMsg = "Method undefined (static/special): " + methodInfo.className + "." + methodInfo.name;
						console.error(errMsg);
						js2me.showError(errMsg);
						throw new Error(errMsg);
					}
	...
	generators[0xc4] = function () {
		...
		throw new Error('wide: unknown op ' + op);
	};
	var program = [];
	var calls = [];
